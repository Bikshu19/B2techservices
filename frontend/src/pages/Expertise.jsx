...
function DomainCard({ domain }) {
  ...
  return (
    <div ref={tiltRef} className="scene cursor-pointer domain-card-container" tabIndex={0} aria-label={`${domain.name} expertise card`}>
    ...
    <div className="relative min-h-screen py-35 px-3 md:px-8 overflow-hidden pro-surface">
      ...
      <section className="relative z-10 mt-24">
        <div className="cap-heading">
          <span className="cap">Why Choose</span>
          <span className="cap-strong">B2 Tech Services?</span>
        </div>
        <p className="cap-sub">
          <span className="grad-amber">Right Job.</span> <span className="grad-amber strong">Right Candidate.</span>{" "}
          <span className="grad-amber strong">Right Now.</span>
        </p>
      </section>
      ...
      <section className="relative z-10 mt-12 px-1">
        <div className="benefit-grid">
          {/* Candidates */}
          <motion.div whileHover={{ y: -8 }} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={cardVariants} className="benefit bordered-panel two-tone">
            <div className="panel-head">
              <div className="panel-icon">👨‍💼</div>
              <h3 className="panel-title text-blue">For Candidates</h3>
              <span className="ribbon ribbon-blue" />
            </div>
            <div className="panel-list">
              {[
                { icon: "🎯", text: "Tailored opportunities matching skills & aspirations" },
                { icon: "⚡", text: "Immediate access to openings suited for you now" },
                { icon: "📈", text: "Career growth with leadership-focused partners" },
              ].map(({ icon, text }, i) => (
                <motion.div key={text} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }} className="panel-item bordered-soft">
          ...
          {/* Employers */}
          <motion.div whileHover={{ y: -8 }} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={cardVariants} className="benefit bordered-panel two-tone alt">
            <div className="panel-head">
              <div className="panel-icon">🏢</div>
              <h3 className="panel-title text-pink">For Employers</h3>
              <span className="ribbon ribbon-pink" />
            </div>
            <div className="panel-list">
              {[
                { icon: "🌟", text: "Access to India's elite, rigorously vetted talent" },
                { icon: "⏱️", text: "Speed and precision in hiring proven to save time" },
                { icon: "📊", text: "Scalable solutions for teams or individual roles" },
              ].map(({ icon, text }, i) => (
                <motion.div key={text} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }} className="panel-item bordered-soft">
                  <span className="pi">{icon}</span>
                  <p>{text}</p>
      ...
      <section className="relative z-10 mt-28">
        <div className="sec-head">
          <h3 className="sec-title">Key Differentiators</h3>
        ...
        <div className="diff-grid">
          {[
            { icon: <FaStar />, label: "Top Hiring Rank", desc: "#1 in tech hiring, leadership, and trust", hue: "gold" },
            { icon: <FaHandshake />, label: "Trusted Relationships", desc: "Integrity and transparency with clients & candidates", hue: "teal" },
            { icon: <FaNetworkWired />, label: "Robust Talent Network", desc: "Elite and diverse talent pool", hue: "indigo" },
            { icon: <FaRocket />, label: "Career-Driven Approach", desc: "Advancing careers, not just filling positions", hue: "violet" },
            { icon: <FaBullseye />, label: "Excellence in Execution", desc: "Quality-driven, speedy and accurate hiring", hue: "rose" },
          ].map(({ icon, label, desc, hue }, index) => (
            <motion.div
              key={label}
              ...
              whileHover={{ y: -8 }}
              className={`diff-card bordered-tile hue-${hue}`}
            >
              <div className="tile-top">
                <div className="medal">{icon}</div>
                <h4 className="tile-title">{label}</h4>
              </div>
      ...
      {/* Metrics */}
      <section className="relative z-10 mt-24">
        <motion.h2 initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="metrics-title">
          Performance Metrics
        </motion.h2>
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 0.6 }} className="metrics-divider" />
        <motion.div initial={{ opacity: 0, scale: 0.94 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7 }} className="metric-card bordered-split">
          <p className="metric-value">50%+</p>
          <p className="metric-label">Hit Ratio</p>
          <p className="metric-note">A 50%+ hit ratio where over 5 of every 10 profiles submitted get shortlisted by clients.</p>
        </motion.div>
        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="metric-foot">
          Measurable results and <span className="strong">strategic talent alignment</span> that drive business success.
        </motion.p>
      </section>
